...
#from velocity.msg import velocity
...
#rospy.init_node('pub_velocity', anonymous=True)
...
def depth_callback(ros_image):
    ...
    vx,vy,vz=avoidance(depth)
    pub.publish(velocity(vx,vy,vz))
    print("pub_vel")
...
def avoidance(image):
    ...
    print(current)
    if (current < 100):
		#client.moveByVelocityZAsync(0, 0, -1.5, 1, airsim.DrivetrainType.ForwardOnly,airsim.YawMode(False, 0)).join()
		...
		#client.moveByVelocityZAsync(vx, vy, -1.5, 1, airsim.DrivetrainType.ForwardOnly,airsim.YawMode(True, 0)).join()
...
def listener():
    ...
    rospy.init_node('process_image_pub_vel', anonymous=True)
